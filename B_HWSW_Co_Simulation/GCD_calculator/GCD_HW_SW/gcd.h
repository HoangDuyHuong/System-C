...
SC_MODULE(Subtractor) {
    ...
    void process() {
        out.write(x.read() >= y.read() ? x.read() - y.read() : y.read() - x.read());
    }
...
SC_MODULE(Controller) {
    ...
    enum states { IDLE, LOAD, COMPARE, SUB_X, SUB_Y, OUTPUT, DONE, WAIT_RELEASE };
    ...
    void next_state_logic() {
        switch (state.read()) {
            ...
            case LOAD:
                ...
                break;
            case COMPARE:
                if (x_eq_y.read()) next_state.write(OUTPUT);
                else if (x_gt_y.read()) next_state.write(SUB_X);
                else next_state.write(SUB_Y);
                break;
            case SUB_X:
            case SUB_Y:
                next_state.write(COMPARE);
            ...
            case OUTPUT:
                ...
                break;
            case DONE:
                next_state.write(WAIT_RELEASE);
                break;
            case WAIT_RELEASE:
                next_state.write(start.read() ? WAIT_RELEASE : IDLE);
                break;
            default:
    ...
    SC_CTOR(Controller) {
        SC_CTHREAD(state_transition, clk.pos());
        reset_signal_is(reset, true);
